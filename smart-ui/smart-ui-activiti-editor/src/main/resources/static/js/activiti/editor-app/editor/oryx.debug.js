...
function DMCommandHandler(nextHandler) {
	...
	/**
	 * Private method to set the next executer in the Chain of Responsibility
	 * (see http://en.wikipedia.org/wiki/Chain-of-responsibility_pattern for
	 * details).
	 * @param {DMCommandHandler} handler The executer that is next in the chain.
	 */
	...
	/**
	 * Invokes the next executer. If there is no next executer, this method
	 ...
	 * If the process() method doesn't catch the command, the next executer will
	 * be invoked.
	 * @param {Object} command The command object to be processed.
	 */
 ...
 * @param {DMCommandHandler} next The executer that is next in the chain.
 */
...
*/	
     ...
     * @param {Function} [eventHandler] An event executer passed to each newly created shape (as eventHandlerCallback)
        ...
        $A(g.childNodes).each((function(path, index){
            if (ORYX.Editor.checkClassType(path, SVGPathElement)) {
				...
				for (var i = 0; i < handler.points.length; i += 2) {
					//executer.points.each((function(point, pIndex){
...
//			console.log("mousevent: %o",event)
    ...
    /**
     * The key executer for this plugin. Every action from the set of cut, copy,
